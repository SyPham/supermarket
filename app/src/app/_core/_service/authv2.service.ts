...
export class Authv2Service implements OnDestroy {
  ...
  currentUser: User;
  roleValue = new BehaviorSubject<any>({ id: 0, name: '' });


  buildingValue = new BehaviorSubject<any[]>([
    {} as any]);
  decodedToken: any;
  profile: any;
  ...
  logOut() {
    ...
    this.buildingValue.next([{} as any]);
    this.roleValue.next({} as any);
  ...
  public setBuildingValue(building: any[]): void {
    this.buildingValue.next(building);
  }
  public getBuildingValue(): Observable<any[]> {
    return this.buildingValue.asObservable();
  }
  public setRoleValue(role: any): void {
    this.roleValue.next(role);
  }
  public getRoleValue(): Observable<any> {
